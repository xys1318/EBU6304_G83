...
function login() {
    ...
    function normalizeRole(inputRole) {
        if (inputRole === "ta") return "applicant";
        if (inputRole === "mo") return "interviewer";
        if (inputRole === "admin") return "sys_admin";
        return inputRole;
    }
    let selectedRole = normalizeRole(role);
    let registeredUser = users.find(u =>
        u.studentId === user &&
        u.password === password &&
        normalizeRole(u.role) === selectedRole
    );
    ...
    if (registeredUser) {
        ...
        isValid = true;
        role = selectedRole;
        redirectUrl = RoleAccess.getLandingPage(role);
    } else if ((role === "applicant" || role === "ta") && user === "ta" && password === "123456") {
        // Default TA account
        // 默认TA账号
        isValid = true;
        role = "applicant";
        redirectUrl = RoleAccess.getLandingPage(role);
    } else if ((role === "interviewer" || role === "mo") && user === "mo" && password === "654321") {
        // Default MO account
        // 默认MO账号
        isValid = true;
        role = "interviewer";
        redirectUrl = RoleAccess.getLandingPage(role);
    } else if (role === "course_lead" && user === "lead" && password === "lead123") {
        isValid = true;
        redirectUrl = RoleAccess.getLandingPage(role);
    } else if ((role === "sys_admin" || role === "admin") && user === "admin" && password === "admin123") {
        // Default Admin account
        // 默认Admin账号
        isValid = true;
        role = "sys_admin";
