window.ChatService = (function () {
    ...
    function getUnreadCount() {
        ...
        if (user.role === "mo") {
            return messages.filter((msg) => msg.toRole === "mo" && !msg.read).length;
        }
        return messages.filter((msg) => msg.toRole === user.role && msg.toUser === user.username && !msg.read).length;
    }

    function getTopChatThread() {
        const user = getCurrentUser();
        const messages = getMessages();
        const mine = messages.filter((m) => user.role === "mo" ? (m.toRole === "mo" || m.fromRole === "mo") : m.taUser === user.username);
    ...
    function markThreadAsRead(jobId, taUser) {
        const user = getCurrentUser();
        const isMo = user.role === "mo";
        const messages = getMessages();
        let changed = false;

        messages.forEach((msg) => {
            const sameThread = String(msg.jobId) === String(jobId) && String(msg.taUser) === String(taUser);
            const forCurrentUser = isMo ? msg.toRole === "mo" : (msg.toRole === user.role && msg.toUser === user.username);
    ...
    function buildThreads() {
        ...
        messages.forEach((msg) => {
            const involved = isMo ? (msg.fromRole === "mo" || msg.toRole === "mo") : (msg.taUser === user.username);
            if (!involved) return;
            ...
            const isUnread = isMo ? (msg.toRole === "mo" && !msg.read) : (msg.toRole === user.role && msg.toUser === user.username && !msg.read);
            if (isUnread) {
                map[key].unread += 1;
            }
        });
